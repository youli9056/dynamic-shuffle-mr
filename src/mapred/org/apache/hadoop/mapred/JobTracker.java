...
public class JobTracker implements MRConstants, InterTrackerProtocol,
  ...
  public synchronized HeartbeatResponse heartbeat(TaskTrackerStatus status, 
    ...
    //这里暂时只是打桩。定时给reduce添加新的partition
   ...
   * @return
   */
  private Map<TaskAttemptID,Long> lastAddPartitionTime = new HashMap<TaskAttemptID,Long>();
  private Map<TaskAttemptID,Integer> lastAddedPartition = new HashMap<TaskAttemptID,Integer>();
  private Map<JobID, Integer> jobAlreadyAssinPartitions = new HashMap<JobID, Integer>();
  private Map<JobID, List<Integer>> jobIdToPartitionsList = new HashMap<JobID,List<Integer>>();
 // private int maxPartition = -1;
//  private int numReduces = -1;
  private int getNextPartition(TaskAttemptID taskId){
	List<Integer> partitionList = jobIdToPartitionsList.get(taskId.getJobID());
	String sid = taskId.getJobID().toString();
	if(partitionList == null)
	{
		int numReduces = jobs.get(taskId.getJobID()).desiredReduces();
		partitionList = Collections.synchronizedList(new LinkedList<Integer>());
		jobIdToPartitionsList.put(taskId.getJobID(), partitionList);
		for(int i = numReduces; i < MapTask.PARTITION_EXPAND_FACTOR*numReduces; i++)
		{
			partitionList.add(i);
		}
	}
	...
	if(partitionList.isEmpty())
		return -1;
	int res = partitionList.remove(0);
	LOG.info("!!!!!!!"+"!!!!!!!!!!assing partition:"+res +" to "+taskId);
	return res;
  ...
  private List<TaskTrackerAction> getAddPartition(String trackerName) {
	  ...
	  Set<TaskAttemptID> taskIds = trackerToTaskMap.get(trackerName);
		...
		for(TaskAttemptID taskId : taskIds){
			...
			long last_time = lastAddPartitionTime.get(taskId) == null?0:lastAddPartitionTime.get(taskId);
			//if(curr_time - last_time >= 1000*30){
				int orign_part = taskidToTIPMap.get(taskId).getIdWithinJob();//给这个reduce task最初指定的partition
				int toArrPart = getNextPartition(taskId);
				boolean finished = toArrPart < 0 
				|| toArrPart >=MapTask.PARTITION_EXPAND_FACTOR*jobs.get(taskId.getJobID()).desiredReduces()-1;
				if(toArrPart < 0)
					addPartitionList.add(new AddPartitionAction(taskId,new int[0],finished));
				else
					addPartitionList.add(new AddPartitionAction(taskId,new int[]{toArrPart},finished));
				lastAddedPartition.put(taskId, toArrPart);
				int alreadAssinPart = jobAlreadyAssinPartitions.get(taskId.getJobID()) == null?
						1:jobAlreadyAssinPartitions.get(taskId.getJobID());
				alreadAssinPart++;
				jobAlreadyAssinPartitions.put(taskId.getJobID(), alreadAssinPart);
				
				if(finished){ 
					for(TaskTrackerAction tta: addPartitionList){
						((AddPartitionAction)tta).setFinished(true);
					}
				}
				
				
			//}
			lastAddPartitionTime.put(taskId, System.currentTimeMillis());
		}
