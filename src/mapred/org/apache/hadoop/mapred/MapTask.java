...
class MapTask extends Task {
	   ...
	   * partition 个数的放大倍数
	   */
	  public static final int PARTITION_EXPAND_FACTOR = 4;
  /**
    ...
    public void collect(K key, V value) throws IOException {
      try {
    	  collector.collect(key, value,
                  partitioner.getPartition(key, value, numPartitions*PARTITION_EXPAND_FACTOR));
  ...
  private class NewOutputCollector<K,V>
    ...
    public void write(K key, V value) throws IOException, InterruptedException {
    	 collector.collect(key, value,
                 partitioner.getPartition(key, value, partitions*PARTITION_EXPAND_FACTOR));
}
  ...
  implements MapOutputCollector<K, V>, IndexedSortable {
    ...
    public MapOutputBuffer(TaskUmbilicalProtocol umbilical, JobConf job,
      ...
      partitions = job.getNumReduceTasks()*MapTask.PARTITION_EXPAND_FACTOR;
      ...
      int maxMemUsage = sortmb << 20;//左移20位,将io.sort.mb配置的以M为单位的转成以Byte为单位的值
      int recordCapacity = (int)(maxMemUsage * recper);//------------------这个是不是索引的比例啊?
      recordCapacity -= recordCapacity % RECSIZE;//----------------------这个recordCapacity是干啥的?
    ...
    public synchronized void collect(K key, V value, int partition
      ...
      try {
        ...
        do {
          ...
          if (kvfull) {
            try {
              while (kvstart != kvend) {//spill完成后skstart==kvend，这里指正在spill过程中
                reporter.progress();
                spillDone.await();
              }
            } catch (InterruptedException e) {
      ...
      try {
        ...
        if (partition < 0 || partition >= partitions*MapTask.PARTITION_EXPAND_FACTOR) {
      ...
      public synchronized void write(byte b[], int off, int len)
        ...
        try {
          do {
            ...
            // sufficient buffer space?从buffindex开始写入
            ...
            if (buffull && !wrap) {
              try {
                while (kvstart != kvend) {
                	//spill thread is running, wait until spill thread finish
                  reporter.progress();
                  spillDone.await();
                }
              } catch (InterruptedException e) {
    ...
    private synchronized void startSpill() {
      ...
      kvend = kvindex;
      bufend = bufmark;
      //从kvstartspill到kvend(从bufstartspill到bufend)
      spillReady.signal();
    }
